...
4. 点击浏览器右上角油猴图标 ➔ 选择 **「添加新脚本」** ➔ 清空默认文字 ➔ 粘贴进去 ➔ **Ctrl+S 保存**。
   * **升级旧版**：在油猴管理面板里点开原来的「ChatGPT Web Codex 本地桥接」，全选粘贴覆盖后保存。不要再「添加新脚本」，两份同名脚本会抢着挂载。
5. 打开或刷新 **https://chatgpt.com/**，网页右下角将立刻浮现悬浮控制胶囊：
   👉 **`🟢 Codex 本地桥: 已连通 (工作区就绪)`**
6. 控制胶囊里还有：**「▶ 执行」**（手动执行最新一条指令；刷新页面后，页面上已有的指令不会自动重跑）、**回传格式**（原版 / 纯文本 / 语法高亮 / 精简标题）、**气泡样式**（回传结果显示成光谱渐变 / 冷色渐变 / 状态栏色块 / 深灰卡片 / 无底色 / 原样蓝色气泡；渐变配色取自 Claude Code 状态栏），选择会自动记住。
...
| **`ChatGPT-Codex-Bridge.user.js`** | 浏览器端 Tampermonkey 脚本（v1.7，适配 2026-09-26 ChatGPT 网页改版） |
| **`诊断_回传.js`** | 排错用：ChatGPT 再改版导致识别不到指令时，粘进 F12 控制台导出页面结构 |
...
* **人工确认模式**：在网页右下角控制岛中，如果取消勾选 **「自动执行并回传」**，AI 的每一步操作都会进入挂起等待状态，只有你点击控制岛上的 **「▶ 执行」** 后才会执行，安全可控；
...
* **v1.7（2026-09-26）**：回传结果的气泡新增三种渐变样式（光谱渐变 / 冷色渐变 / 状态栏色块），默认光谱渐变。
* **v1.6（2026-09-26）**：ChatGPT 当天网页改版后，消息不再带角色标记，代码块也不再用 `<pre>`，输入框的选择器同样变了，v1.2 因此识别不到任何指令。本版改为以"最后一条回传结果之后的最后一个工具代码块"判定待执行指令，并修复以下问题：同一指令再次出现会被跳过、报错被 4 秒一次的心跳覆盖、发送按钮还没就绪就点击、回传结果里的代码框被误当成新指令、读取含 ``` 的文件时代码框被提前截断。同时新增「▶ 执行」按钮、回传格式切换和气泡样式切换，并附 `诊断_回传.js` 排错片段。
* **v1.2**：初版发布。
